...
RegisterServerEvent('hospital:server:BillOut')
AddEventHandler('hospital:server:BillOut', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	Player.Functions.RemoveMoney("bank", Config.BillCost, "bill-out")
	TriggerEvent("qb-bossmenu:server:addAccountMoney", "ambulance", Config.BillCost)
	TriggerClientEvent('qb-phone:client:RemoveBankMoney', Player.PlayerData.source, Config.BillCost)
...
AddEventHandler('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		...
		TriggerClientEvent('DoLongHudText', src, 'All your possessions have been taken..', 2)
		TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
		return
	end
end)
...
RegisterServerEvent('hospital:server:SetPatientStatus')
AddEventHandler('hospital:server:SetPatientStatus', function(bool)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player ~= nil then
		Player.Functions.SetMetaData("patient", bool)
		TriggerClientEvent("QBCore:client:patient", src, bool)
	end
end)

RegisterServerEvent('hospital:server:SetInjuredStatus')
AddEventHandler('hospital:server:SetInjuredStatus', function(injuredTime)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.SetMetaData("injured", injuredTime)
end)
...
AddEventHandler('hospital:server:SendDoctorAlert', function()
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		local Player = QBCore.Functions.GetPlayer(v)
		if Player ~= nil then 
			if (Player.PlayerData.job.name == "ambualnce" and Player.PlayerData.job.onduty) then
...
QBCore.Commands.Add("checkstatus", "Check a person's health (EMS Only)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", source)
	else
		TriggerClientEvent('QBCore:Notify', source, 'This Command Is For EMS', 'error')
	end
end)
...
QBCore.Commands.Add("heal", "Help a person's injuries (EMS Only)", {}, false, function(source, args)
	...
	if Player.PlayerData.job.name == "ambulance" then
		if Player.Functions.GetItemByName('bandage') ~= nil then
			...
			TriggerClientEvent('QBCore:Notify', source, 'You Dont Have A Bandage', 'error')
		end
	else
		TriggerClientEvent('QBCore:Notify', source, 'This Command Is For EMS', 'error')
	end
...
QBCore.Commands.Add("revivep", "Help a person up (EMS Only)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
		if Player.Functions.GetItemByName('medkit') ~= nil then
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('DoShortHudText', helperId, "You cant help this person..", 2)
	end
end)